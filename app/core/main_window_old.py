...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        for title in pages[5:]:
            self.stack.addWidget(
                self.create_placeholder_page(title)
            )
...
self.update_hardware()
    ...
    def create_dashboard(self):
        ...
        self.gpu_details.setStyleSheet(
            "font-size: 15px;"
        )
    ...
    def create_monitoring_page(self):
        ...
        layout.addWidget(
            QLabel("Real-time system performance")
        )
        ...
        self.monitor_download_label = QLabel(
            "Download: 0 Mbps"
        )
        self.monitor_upload_label = QLabel(
            "Upload: 0 Mbps"
        )
        ...
        self.download_graph = GraphWidget(
            "Download (Mbps)", 0, 100
        )
        self.upload_graph = GraphWidget(
            "Upload (Mbps)", 0, 100
        )
    ...
    def update_monitoring(self):
        ...
        cpu = data["cpu"]
        ram = data["ram"]
        download = self.monitor.bytes_to_mbps(
            data["download"]
        )
        upload = self.monitor.bytes_to_mbps(
            data["upload"]
        )

        self.monitor_cpu_label.setText(
            f"CPU: {cpu:.1f}%"
        )
        self.monitor_ram_label.setText(
            f"RAM: {ram:.1f}%"
        )
        self.monitor_download_label.setText(
            f"Download: {download:.3f} Mbps"
        )
        self.monitor_upload_label.setText(
            f"Upload: {upload:.3f} Mbps"
        )

        self.cpu_graph.set_values(
            self.monitor.cpu_history
        )
        self.ram_graph.set_values(
            self.monitor.ram_history
        )
        ...
        self.download_graph.set_values(
            download_history
        )
        self.upload_graph.set_values(
            upload_history
        )
    ...
    def create_hardware_page(self):
        ...
        layout.addWidget(
            QLabel("Detailed hardware information")
        )

        self.hardware_cpu = self.add_hardware_frame(
            layout,
            "Processor"
        )
        self.hardware_ram = self.add_hardware_frame(
            layout,
            "Memory"
        )
        self.hardware_system = self.add_hardware_frame(
            layout,
            "System"
        )
        self.hardware_gpu = self.add_hardware_frame(
            layout,
            "Graphics"
        )
    ...
    def update_hardware(self):
        ...
        self.hardware_cpu.setText(
            ...
            f"Current frequency: "
            f"{cpu['frequency_current'] / 1000:.2f} GHz\n"
            f"Maximum frequency: "
            f"{cpu['frequency_max'] / 1000:.2f} GHz"
        )

        self.hardware_ram.setText(
            f"Total memory: {ram['total_gb']:.2f} GB"
        )

        self.hardware_system.setText(
            f"Computer: {system['computer']}\n"
            f"OS: {system['name']} {system['release']}\n"
    ...
    def create_storage_page(self):
        ...
        layout.addWidget(
            QLabel("Disk usage and available space")
        )
    ...
    def update_network_page(self):
        ...
        if not networks:
            self.network_layout.addWidget(
                QLabel("No network interfaces detected")
            )
            return

        networks = sorted(
            networks,
            key=lambda x: not x["is_up"]
        )
        ...
        for network in networks:
            ...
            details = QLabel(
                ...
                f"MAC: {network['mac']}\n"
                f"Interface speed: "
                f"{network['speed_mbps']} Mbps\n"
                f"Sent: "
                f"{self.format_bytes(network['bytes_sent'])}\n"
                f"Received: "
